  ...
  const rosbag2_transport::RecordOptions & record_options)
{
  ...
  auto & [all_topics, all_services, is_discovery_disabled,
    ...
    start_paused, use_sim_time, disable_keyboard_controls, timeout_for_delay] = record_options;
  Node node;
  ...
  node["timeout_for_delay"] = timeout_for_delay;
  return node;
}
  ...
  const Node & node, rosbag2_transport::RecordOptions & record_options, int version)
{
  ...
  optional_assign<float>(node, "timeout_for_delay", timeout_for_delay);
  return true;
}
